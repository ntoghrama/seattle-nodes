...
#rootpath= '/mnt/data0/UW_xiaotao_stns_plus_test_stns'
rootpath = '/mnt/data1/SEATTLE_June-2021_studies'
CCFDIR = os.path.join(rootpath,'CCFs-S1-output_40-Hz_one-bit_july-2022')

#CCFDIR = os.path.join(rootpath,'CCFs-S1-output_40-Hz_laura-corrected-Sept-2021')
#DATADIR = os.path.join(rootpath,'RAW_DATA') # Event_2019_07_15
#DATADIR='/mnt/data0/SEATTLE_NODES_DATA/JUNE_2021_studies/NODES_mseed_data_downsampled_20_Hz/'
DATADIR = os.path.join(rootpath,'NODES_asdf_data_40_Hz')
#locations = os.path.join(rootpath,'station.txt')
locations = '/mnt/data1/SEATTLE_June-2021_studies/STATION_TEXTFILES--study_areas/station.txt'
...
## NoisePy location: /mnt/data3/user/NoisePy-master-June2021

# some control parameters
input_fmt   = 'asdf'                                           # string: 'asdf', 'sac','mseed' 
freq_norm   = 'one_bit'      # whiten with rma Sept 2021 ACFs and CCFs     # 'no' for no whitening, or 'rma' for running-mean average, 'phase' for sign-bit normalization in freq domain
...
cc_len    = 1200           # window length                                      # basic unit of data length for fft (sec)
...
maxlag         = 100       #for xiaotao uw, 200+ s          # lags of cross-correlation to save (sec)
substack       = True                                       # sub-stack daily cross-correlation or not
substack_len   = 10800   #36*cc_len     # tried 1, 10, 36       # how long to stack over: need to be multiples of cc_len
...
if input_fmt == 'asdf':
    ...
    #dfile = os.path.join(DATADIR,'download_info--missing-stns-May-2022.txt')
...
######################################
    ...
    if os.path.isfile(tmpfile):
        ...
        if len(alines) and alines[-1] == 'done':
            continue
            #print('hi') # natasha -- remove the above continue for when you are appending a few stations to already processed and output correlation functions
        else:
            ftemp.close()
            os.remove(tmpfile)
########################################
    ...
    for ista in range(len(sta_list)):
        ...
        if input_fmt == 'asdf':
            ...
            #if len(all_tags)==0:print('EXIT!');sys.exit()    # natasha
    ...
    #for iiS in range(3):
        ...
        print("smoothed FFT is: "+str(sfft1))   # natasha
        t1=time.time()
        if flag: 
...
#                    iend=np.minimum(iiS+ncomp,iii) 
        #iend=np.minimum(iiS+ncomp,iii) 
        #if(source==receiver) is False:


        print("istart is "+str(istart)) # marine
        print("iend is "+str(iend)) # marine
        print(ncomp)    #marine
...
#                    istart=np.minimum(iiS+ncomp,iii)
        ...
        for iiR in range(istart,iend):
            ...
            if acorr_only: # marine
                if (station[iiS]!=station[iiR]):continue # marine

            if flag:print('receiver: %s %s %s' % (station[iiR],network[iiR],channel[iiR])) # marine
            if flag:print('source: %s %s %s' % (station[iiS],network[iiS],channel[iiS])) # marine
            
            #print("station length is: " +str(station[istart:iend])) #marine
            #print(channel[istart:iend])
            #if flag:print('receiver: %s %s' % (station[iiR],network[iiR]))
            #if flag:print('source: %s %s' % (station[iiS],network[iiS]))    # natasha
            ...
            print("cc_h5 file is: "+str(cc_h5))    # natasha
            ...
            with pyasdf.ASDFDataSet(cc_h5,mpi=False) as ccf_ds:
                ...
                print("data type",data_type,"path",path) # natasha 
                crap[:] = corr[:]
                print(crap.shape) # natasha
