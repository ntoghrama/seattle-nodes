...
freqs = [[1.,2.],[0.5,1.],[2,4]]
# freqmin = 1. # or hard-code these values and remove loop.
# freqmax = 2.  ## 0.5-3 Hz general range for these CCFs.

amp_reduce_factor = 15. # value by which to divide waveforms for visualization, to make plotting clearer

# distance bins:
dist_bin_mins = np.arange(0,5.8,0.05) # 0.,5.8.,0.1 -- 5.9 km is max interstation distance bound; 0.05, bin every 50m
...
# for 0-6 km, 1-2 Hz -- 17 sec disp_lag
disp_lag = 17

ccomps = ['ZZ','TT','RR','RT','RZ','TR','TZ','ZR','ZT']

# mm=['mean','median'] ## could loop through both, for stack_mean_or_median in mm:
...
for ff in freqs:
    ...
    for ccomp in ccomps:
        ...
        for dist_bin_min in dist_bin_mins:
            ...
            plt.text(disp_lag+0.4, bin_avg_dist, f'{len(waveforms)}',fontsize=2)
        ...
        plt.savefig(outdir+f'/{ccomp}-moveout_1D_ALL-stn-pairs_{stack_method}--binned-by-{dist_bin_size}m--stack-by-{stack_mean_or_median}--{freqmin}-{freqmax}-Hz--amp-downscaled-by-{str(int(amp_reduce_factor))}.png', format='png', dpi=400)
        plt.cla()
        # plt.show()