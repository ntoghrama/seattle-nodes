    ...
    # have to read each sac/mseed data one by one
        ...
        if messydata:
            ...
            for ii in range(nfiles):
                ...
                #print(allfiles[0],year,month,day)
                #sys.exit('bye bye')
                #print(allfiles[0],year,month,day)
                #print(allfiles[ii],year,month,day)
...
def stats2inv(stats,prepro_para,locs=None):
    ...
    if input_fmt=='sac':
        ...
        #print(locs[locs['station']==np.int64(stats.station)]).index.values.astype('int64')
        #sys.exit('bye bye')
        print('stats are: ')
        print(stats)
        #print(stats.station)
        ...
        print('stn that will be written into inventory: ',str(sta)) # natasha
...
def noise_processing(fft_para,dataS):
    ...
    if freq_norm != 'no':
        ...
        source_white = scipy.fftpack.fft(white, Nfft, axis=1) # return FFT    # commented out, laura-marine-natasha test July 2021
        #source_white = scipy.fftpack.fft(white, axis=1)
    return source_white
...
def correlate(fft1_smoothed_abs,fft2,D,Nfft,dataS_t):
    ...
    if substack:
        if substack_len == cc_len:
            ...
            for i in range(nwin): 
                n_corr[i]= 1           
                #crap[:Nfft2] = corr[i,:] # removed laura test
                crap[:Nfft2] = corr[i,:]
                #crap[:Nfft2] = crap[:Nfft2]-np.mean(crap[:Nfft2])   # remove the mean in freq domain (spike at t=0) #muted-laura-marine july 2021
                #crap[-(Nfft2)+1:] = np.flip(np.conj(crap[1:(Nfft2)]),axis=0)
                crap[(Nfft2):] = np.flip(np.conj(crap[1:(Nfft2+1)]),axis=0)
                crap[0]=complex(0,0)
                #s_corr[i,:] = np.real(np.fft.ifftshift(scipy.fftpack.ifft(crap, Nfft, axis=0)))
                s_corr[i,:Nfft-1] = np.fft.ifftshift(scipy.fftpack.ifft(crap, Nfft, axis=0))[1:Nfft]
