...
#CCFDIR = os.path.join(rootpath,'CCFs')
#CCFDIR = os.path.join(rootpath,'ACFs-S1-output_40-Hz_laura-corrected-Sept-2021')
CCFDIR = os.path.join(rootpath,'CCFs-S1-output_40-Hz_one-bit_july-2022')

#rootpath= '/mnt/data0/UW_Xiaotao_stns'
STACKDIR = os.path.join(rootpath,'STACK_CCFs--S2-output_one-bit--August-2022--rotated-RTZ--Feb-2023')
#locations = os.path.join(rootpath,'UW_xiaotao_7nodes_station.txt')
locations = os.path.join(rootpath,'station.txt')

#locations = os.path.join(rootpath,'xiaotao_plus_UW_test_stns.txt')
#rootpath = '/mnt/data1/CAUCASUS_XA_DATA_hydro'
#rootpath  = '/mnt/data0/SEATTLE_NODES_DATA/'                                # root path for this data processing
#CCFDIR = '/mnt/data0/seattle_broadbands_2019/CCFs_7nodes_UW_BST' 
#CCFDIR    = os.path.join(rootpath,'CCF_Nov24_2019')                            # dir where CC data is stored
#CCFDIR = os.path.join(rootpath,'CCFs_autumn')
#STACKDIR  = os.path.join(rootpath,'STACK_Nov24_2019/pws_stack')                          # dir where stacked data is going to
#STACKDIR = '/mnt/data1/CAUCASUS_XA_DATA_hydro/STACK_CCFs_autumn'
#locations = os.path.join(rootpath,'station_caucasus.txt')
#locations = os.path.join(rootpath,'station.txt')           # station info including network,station,channel,latitude,longitude,elevation 
#locations = '/mnt/data0/master_HarvardBB_nodes_UW_HH_station.txt'
#locations='/mnt/data0/7nodes_UW_BST_station.txt'
#locations = os.path.join(rootpath,'UW_Xiaotao_locations.txt')
...
if rank == 0:
    ...
    sta = []
    for ii in range(len(tlocs)): 
        sta.append(str(tlocs['network'][ii])+'.'+str(tlocs['station'][ii]))
    sta = np.unique(sta)
    #sta = sorted(np.unique(str(tlocs['network'])+'.'+str(tlocs['station'])))
    #sta = sorted(np.unique('station_'+str(tlocs['station'])))
...
for ipair in range (rank,splits,size):
    ...
    for ifile in ccfiles:
        ...
        # can try commenting this section out:    # comment out; Natasha
