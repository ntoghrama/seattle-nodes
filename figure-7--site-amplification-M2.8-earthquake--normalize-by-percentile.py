...
def main():
    ...
    for cbar_option in ["capped", "natural"]:
        ...
        if cbar_option == "capped":
            ...
            combined_filename = f"{quake}--normalized-energy-maps_{NORM_PCTL}-pctl--cbar-capped-{cbar_cap}--window-{window_start}-{window_end}-s.png"
            print(combined_filename)
            fig.tight_layout(rect=[0.04, 0.02, 0.86, 0.91], w_pad=0.6)
        else:
            combined_filename = f"{quake}--normalized-energy-maps_{NORM_PCTL}-pctl--natural-cbars--window-{window_start}-{window_end}-s.png"
