...
def make_timestamps(prepro_para):
    ...
    # have to read each sac/mseed data one by one
        ...
        if messydata:
            ...
            for ii in range(nfiles):
                #year = int(allfiles[ii].split('/')[-1].split('_')[1][0:4])  # for Harvard broadbands
                #month = int(allfiles[ii].split('/')[-1].split('_')[1][4:6])  # for Harvard broadbands
                #day = int(allfiles[ii].split('/')[-1].split('_')[1][6:])  # for Harvard broadbands
                year = int(allfiles[ii].split('..')[-1][0:4])       # for NODES in mseed
                month = int(allfiles[ii].split('..')[-1].split('-')[1])  # for NODES in mseed
                day =int(allfiles[ii].split('..')[-1].split('-')[2][0:2]) # for NODES in mseed
...
def preprocess_raw(st,inv,prepro_para,date_info):
    ...
    if rm_resp != 'no':
        ...
        if rm_resp == 'inv':
            ...
            specfile = glob.glob(os.path.join(respdir,'resp.'+station+'*.npy'))
            if len(specfile)==0:
                raise ValueError('no response spectrum found for %s' % station)
            st = resp_spectrum(st,specfile[0],samp_freq,pre_filt)

        elif rm_resp == 'RESP':
            print('remove response using RESP files')
            resp = glob.glob(os.path.join(respdir,station+'*'+channel+'*'+'.RESP'))
            if len(resp)==0:
                raise ValueError('no RESP files found for %s' % station)
            print(date_info)
            seedresp = {'filename':resp,'date':date_info['starttime'],'units':'DIS'}
            st.simulate(paz_remove=seedresp,pre_filt=pre_filt)
...
def stats2inv(stats,prepro_para,locs=None):
    ...
    if input_fmt=='sac':
        ...
        print(np.int64(stats.station))    # use this line to check print for nodes
        ista=locs[locs['station']==np.int64(stats.station)].index.values.astype('int64')[0] # for station names that are INTEGERS (NODES data set)
        # ista=locs[locs['station']==np.str(stats.station)].index.values.astype('int64')[0]    # for station names that are STRINGS
        # print('ista is: ',ista)
...
def cut_trace_make_statis(fc_para,source):
    ...
    all_madS = mad(data)	            # median absolute deviation over full noise window
    all_stdS = np.std(data)	        # standard deviation over full noise window
...
def correlate(fft1_smoothed_abs,fft2,D,Nfft,dataS_t):
    ...
    if substack:
        if substack_len == cc_len:
            ...
            interim_item   = np.zeros(Nfft,dtype=np.complex64)
            for i in range(nwin): 
                n_corr[i]= 1           
                interim_item[:Nfft2] = corr[i,:]
                interim_item[(Nfft2):] = np.flip(np.conj(interim_item[1:(Nfft2+1)]),axis=0)
                interim_item[0]=complex(0,0)
                s_corr[i,:Nfft-1] = np.fft.ifftshift(scipy.fftpack.ifft(interim_item, Nfft, axis=0))[1:Nfft]

            # remove abnormal data
            ampmax = np.max(s_corr,axis=1)
            tindx  = np.where( (ampmax<20*np.median(ampmax)) & (ampmax>0))[0]
            s_corr = s_corr[tindx,:]
            print("scorr_size is "+str(s_corr.size))
            t_corr = t_corr[tindx]
            n_corr = n_corr[tindx]
            ...
            for istack in range(nstack):                                                                   
                ...
                interim_item[:Nfft2] = np.mean(corr[itime,:],axis=0)   # linear average of the correlation 
                interim_item[:Nfft2] = interim_item[:Nfft2]-np.mean(interim_item[:Nfft2])   # remove the mean in freq domain (spike at t=0)
                interim_item[-(Nfft2)+1:]=np.flip(np.conj(interim_item[1:(Nfft2)]),axis=0)
                interim_item[0]=complex(0,0)
                s_corr[istack,:] = np.real(np.fft.ifftshift(scipy.fftpack.ifft(interim_item, Nfft, axis=0)))
        ...
        t_corr = dataS_t[0]
        interim_item   = np.zeros(Nfft,dtype=np.complex64)
        interim_item[:Nfft2] = np.mean(corr[tindx],axis=0)
        interim_item[:Nfft2] = interim_item[:Nfft2]-np.mean(interim_item[:Nfft2],axis=0)
        interim_item[-(Nfft2)+1:]=np.flip(np.conj(interim_item[1:(Nfft2)]),axis=0)
        s_corr = np.real(np.fft.ifftshift(scipy.fftpack.ifft(interim_item, Nfft, axis=0)))
...
def correlate_nonlinear_stack(fft1_smoothed_abs,fft2,D,Nfft,dataS_t):
    ...
    for i in range(nwin): 
        ...
        interim_item[-(Nfft2)+1:] = np.flip(np.conj(interim_item[1:(Nfft2)]),axis=0)
        interim_item[0]=complex(0,0)
        s_corr[i,:] = np.real(np.fft.ifftshift(scipy.fftpack.ifft(interim_item, Nfft, axis=0)))
    ...
    if substack:
        ...
        if stack_method == 'linear':
            ...
            s_corr,_,_ = robust_stack(s_corr,0.001)
            t_corr = dataS_t[0]
            n_corr = nwin       
...
def stacking(cc_array,cc_time,cc_ngood,stack_para):
    ...
    if not len(tindx):
        ...
        if smethod == 'linear':
            ...
            allstacks1,_,_ = robust_stack(cc_array,0.001)
        elif smethod == 'all':
            allstacks1 = np.mean(cc_array,axis=0)
            allstacks2 = pws(cc_array,samp_freq) 
            allstacks3,_,_ = robust_stack(cc_array,0.001)
...
def rotation(bigstack,parameters,locs,flag):
    ...
    if ncomp<9:
        print('interim_item did not get enough components')
        tcorr=[]
        return tcorr
...
def segment_interpolate(sig1,nfric):
    ...
    #----instead of shifting, do an interpolation------
...
def robust_stack(cc_array,epsilon):
    ...
    while res > epsilon:
        ...
        for i in range(cc_array.shape[0]):
            interim_item = np.multiply(stack,cc_array[i,:].T)
            interim_item_dot = np.sum(interim_item)
            di_norm = np.linalg.norm(cc_array[i,:])
            ri = cc_array[i,:] -  interim_item_dot*stack
            ri_norm = np.linalg.norm(ri)
            w[i]  = np.abs(interim_item_dot) /di_norm/ri_norm #/len(cc_array[:,1])
        # print(w)
        w =w /np.sum(w)
