    ...
    for bp in cluster_filters: ## bp = string of filtering freq band. plot by freq band.
        ...
        for cl in cluster_labels:
            ...
            for i, ff in enumerate(files):      
                ...
                plt.fill_between(amplifier*(demeaned_norm[4000:])+((lats[i]-47.5)*1112.5),x_axis[4000:],color='k',alpha=a_value)
                plt.title("Linear stack ACFs, "+str(study_area)+', '+str(channel)+" component, mean removed, renormalized, "+str(freqmin)+'-'+str(freqmax)+' Hz, cluster '+str(cl.split('l')[-1])+', '+cluster_name+', amplified x'+str(amplifier)) 
