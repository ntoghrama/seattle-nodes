...
display_sec_start = 6.0  
display_sec_end = 28.0 
event_time_utc = UTCDateTime("2019-07-17T23:43:17")  # UTC event time

en_window_start, en_window_end = 15, 17
z_window_start, z_window_end = 8, 10
...
legend_elements = [
    ...
    Line2D([0], [0], marker='o', color='w', markerfacecolor='magenta',
           markersize=6, markeredgecolor='navy', markeredgewidth=0.5, linestyle='None',
           label=f'P-wave peak amplitude ({z_window_start}–{z_window_end} s)'),
    Line2D([0], [0], marker='o', color='cyan', markersize=6, markeredgewidth=0.75,
           label=f'S-wave peak amplitude ({en_window_start}–{en_window_end} s)', markeredgecolor='navy', linestyle='None'),
    Line2D([0], [0], marker='s', color='cyan', markersize=5, markeredgewidth=0.75,
           markeredgecolor='navy', linestyle='None', label='Horizontal component S-wave peak amplitudes plotted on Z'),
]


fig, axs = plt.subplots(nrows=1, ncols=3,figsize=(12,7), sharex=True, sharey=True)
...
for i, component in enumerate(components):
        ...
        try:
            ...
            if len(st) > 0:
                ...
                if norm_factor > 0:
                    ...
                    # P wave peak markers
                    if component in ["DPZ"]:
                        window_indices = np.where((times >= z_window_start) & (times <= z_window_end))[0]
                        if window_indices.size > 0:
                            window_vals = normalized_env[window_indices]
                            chosen_idx = window_indices[np.argmax(window_vals)]
                            ax.plot(times[chosen_idx],
                                    normalized_env[chosen_idx] + lat * lat_scale + 10,
                                    'o', markersize=4, color='magenta',
                                    markeredgecolor='navy', markeredgewidth=.6,
                                    zorder=10)
                        # load E peaks and plot as squares on Z component:
                        try:
                            files_E = glob.glob(f"{data_dir}/**/IRISPH5-Z6.{station}.DP2..{event_date}T*.mseed", recursive=True)
                            if files_E:
                                st_E = read(files_E[0])
                                st_E.trim(starttime=event_time_utc - 10, endtime=event_time_utc + 60)
                                st_E.detrend("demean")
                                st_E.filter("bandpass", freqmin=minfreq, freqmax=maxfreq, corners=4, zerophase=True)
                                trace_E = st_E[0]
                                times_E = trace_E.times(reftime=event_time_utc)
                                env_E = envelope(trace_E.data)
                                norm_factor_E = np.percentile(np.abs(trace_E.data), 99)
                                if norm_factor_E > 0:
                                    normalized_env_E = (env_E / norm_factor_E) * amplitude_scale
                                    e_window_indices = np.where((times_E >= en_window_start) & (times_E <= en_window_end))[0]
                                    if e_window_indices.size > 0:
                                        e_peak_idx = e_window_indices[np.argmax(normalized_env_E[e_window_indices])]
                                        ax.plot(times_E[e_peak_idx],
                                                normalized_env_E[e_peak_idx] + lat * lat_scale + 10,
                                                marker='s', markersize=3, color='cyan', markeredgecolor='navy',
                                                markeredgewidth=0.7, zorder=12)
                        except Exception as e:
                            print(f"Error processing E channel for station {station}: {e}")

                        # load N peaks and plot as squares on Z component:
                        try:
                            files_N = glob.glob(f"{data_dir}/**/IRISPH5-Z6.{station}.DP1..{event_date}T*.mseed", recursive=True)
                            if files_N:
                                st_N = read(files_N[0])
                                st_N.trim(starttime=event_time_utc - 10, endtime=event_time_utc + 60)
                                st_N.detrend("demean")
                                st_N.filter("bandpass", freqmin=minfreq, freqmax=maxfreq, corners=4, zerophase=True)
                                trace_N = st_N[0]
                                times_N = trace_N.times(reftime=event_time_utc)
                                env_N = envelope(trace_N.data)
                                norm_factor_N = np.percentile(np.abs(trace_N.data), 99)
                                if norm_factor_N > 0:
                                    normalized_env_N = (env_N / norm_factor_N) * amplitude_scale
                                    n_window_indices = np.where((times_N >= en_window_start) & (times_N <= en_window_end))[0]
                                    if n_window_indices.size > 0:
                                        n_peak_idx = n_window_indices[np.argmax(normalized_env_N[n_window_indices])]
                                        ax.plot(times_N[n_peak_idx],
                                                normalized_env_N[n_peak_idx] + lat * lat_scale + 10,
                                                marker='s', markersize=3, color='cyan', markeredgecolor='navy',
                                                markeredgewidth=0.7, zorder=12)
                        except Exception as e:
                            print(f"Error processing N channel for station {station}: {e}")
    ...
    # leftmost subplot - latitude axis
    if i == 0:
...
if "longitude" in stations_df.columns:
    ...
    arrow_length = 0.4  # Adjust this value if needed
...
fig.suptitle("M3.0 Cathcart Earthquake", fontsize=15, fontweight='bold', y=0.99)
fig.supxlabel("Time after earthquake (seconds)", fontsize=12, y=0.12)

fig.legend(handles=legend_elements,loc='lower center',bbox_to_anchor=(0.5, 0.05),fontsize=8,framealpha=0.97,ncol=3)

env_tag = "plot-envelope" if plot_envelope else "no-env"
filename = (f"M3.0--arrival-peaks-maps-1x3--{minfreq}-{maxfreq}-Hz--"
            f"{display_sec_start}-{display_sec_end}-sec-EN-window-"
            f"{en_window_start}-{en_window_end}-sec-EN-window--{z_window_start}-{z_window_end}-Z-window--peaks-marked--{env_tag}--backaz-arrow.png")
